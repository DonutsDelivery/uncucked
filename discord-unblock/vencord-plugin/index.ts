/*
 * Vencord userplugin: NSFW Unblocker
 * Removes NSFW age gates and explicit content filters
 *
 * Install: copy this folder to Vencord/src/userplugins/nsfwUnblocker/
 * Then run: pnpm build
 */
...
// ─── XHR intercept ───────────────────────────────────────────────
// Discord sends X-Super-Properties header which includes age verification
// state. Server returns 403 for NSFW channel messages if not verified.
...
export default definePlugin({
    ...
    start() {
        // XHR intercept — bypass X-Super-Properties 403 on NSFW channels
        patchXHR();

        // Runtime user patching
        patchUser();
